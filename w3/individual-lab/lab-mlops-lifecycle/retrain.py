...
def train_model_on_df(df: pd.DataFrame, contamination: float = 0.03, n_estimators: int = 100):
    """Train a StandardScaler+IsolationForest pipeline on a DataFrame, return (model, metrics)."""
    ...
    return model, anomaly_rate, len(X)
...
def reload_serve(serve_url: str) -> None:
    try:
        resp = requests.post(f"{serve_url}/reload", timeout=60)
...
def main():
    ...
    # v2 now holds @production; clear the now-stale @staging alias.
    try:
