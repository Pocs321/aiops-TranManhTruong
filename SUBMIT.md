...
## Knowledge Check (đáp án tham khảo — cần viết tay & chụp ảnh)

> Đính kèm ảnh viết tay tại `figures/knowledge_check.jpg`. Đây là các ý cần viết.

1. **Skewness & 3σ.** Skewness = độ bất đối xứng của phân phối. Lệch phải (>0): đuôi phải
   dài (latency); lệch trái (<0): đuôi trái dài. 3σ giả định đối xứng, nên trên data skewed
   mean bị kéo về phía đuôi và σ bị thổi phồng → ngưỡng phía đuôi *quá xa* (bỏ sót anomaly
   thật) còn phía kia có thể ra *âm/vô nghĩa*. **Cách xử lý:** (a) log-transform data lệch
   phải (`np.log1p`) cho đối xứng rồi mới dùng 3σ; (b) dùng IQR (median + Q1/Q3, theo
   percentile, bỏ qua đuôi) hoặc Isolation Forest (không giả định phân phối).

2. **3σ vs EWMA vs STL.**
   - **3σ (rolling z-score):** bắt spike/drop đột ngột trên data stationary gần Gaussian.
     Fail trên data seasonal (false alarm, hoặc cửa sổ rộng → miss) và trên drift.
   - **EWMA:** bắt **drift** chậm (memory leak) nhờ trí nhớ mũ dài. Fail với spike sắc nhọn
     và với data seasonal.
   - **STL:** bỏ trend + seasonal, phát hiện trên residual. Tốt nhất cho metric **seasonal**
     ngày/tuần. Cần đúng `period`; nặng hơn.
3. **Isolation Forest "path ngắn = anomaly".** Random feature/ngưỡng để cắt, lặp tới khi
   tách riêng từng điểm; outlier nằm xa nên bị tách sau **ít** lần cắt (path ngắn), điểm
   bình thường cần nhiều. **Cần feature engineering** vì IF xem mỗi dòng độc lập — `value=80`
   không có ngữ cảnh thời gian. Thêm rolling mean/std, rate of change, lag, giờ/thứ đưa ngữ
   cảnh vào để phân biệt spike với một giá trị cao bình thường.
4. **Univariate vs multivariate (memory leak).** Xét riêng từng metric đều ổn: memory tăng
   chậm (rolling band trôi theo), CPU phẳng/bình thường, GC/latency dưới ngưỡng tĩnh →
   univariate **bỏ sót** tới khi crash. IF multivariate thấy **tổ hợp bất thường** —
   memory↑ + GC↑ + latency↑ trong khi CPU phẳng (bình thường load↑ thì CPU cũng↑) — tương
   quan chưa từng thấy → **bắt sớm** (giờ 4–6 thay vì 12–14).
5. **Precision vs Recall trong AIOps.** Precision = trong các alert báo ra, bao nhiêu là
   thật; Recall = trong các anomaly thật, bắt được bao nhiêu. AIOps ưu tiên **recall**: bỏ
   sót một outage = SLA breach / mất doanh thu / postmortem, còn false alarm tốn ~5 phút để
   dismiss → cost(miss) ≫ cost(false alarm). **Đánh đổi:** hạ threshold để tăng recall cũng
   làm tăng false alarm (precision giảm) → alert fatigue; tune thiên recall nhưng canh
   false-alarm rate để on-call không bắt đầu ignore alert.
